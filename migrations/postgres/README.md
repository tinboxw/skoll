...
- 金额：最终金额列使用 `NUMERIC(18,2)`，聚合行 JSON 保留当前领域数量与单价语义。
...
- 金额：合同使用 `NUMERIC(18,2)`；机会、付款和发票金额使用 `BIGINT` 分值。
...
### 20260727_000032_add_governed_workflow_routing.sql

- 为审批节点增加显式 `any`、`all`、`quorum` 决策策略和法定人数。
- 为流转边增加有界结构化条件，并为实例保存启动变量与并行活动节点。
- 当前规则：审批节点必须声明决策策略；条件、并行分支和法定人数只走当前工作流状态机。

### 20260727_000033_add_workflow_assignment_governance.sql

- 为审批节点增加有界升级策略，为任务保存原审批人、分配类型与授权来源。
