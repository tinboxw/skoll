...
const columns: DataTableColumn[] = [
	{ key: "title", label: t("pharma.salesOpportunity.opportunity"), minWidth: 190 },
	{ key: "customerText", label: t("pharma.salesOpportunity.customer"), minWidth: 210 },
	{ key: "productText", label: t("pharma.salesOpportunity.products"), minWidth: 190 },
	{ key: "amountText", label: t("pharma.salesOpportunity.expected"), width: 130 },
	{ key: "closeText", label: t("pharma.salesOpportunity.closeDate"), width: 130 },
	{ key: "stage", label: t("pharma.salesOpportunity.stage"), width: 125 }
];
...
async function save() {
	...
	try {
		...
		ElMessage.success(editingId.value ? t("pharma.salesOpportunity.opportunityUpdated") : t("pharma.salesOpportunity.opportunityCreated"));
	} catch (cause) {
		error.value = toErrorMessage(cause);
	} finally {
		saving.value = false;
...
async function applyStage(item: SalesOpportunity, stage: SalesOpportunityStage, note: string) {
	...
	try {
		...
		await refreshStatistics();
		ElMessage.success(stage === "lost" ? t("pharma.salesOpportunity.closeAsLost") : `${t("pharma.salesOpportunity.advanceOpportunity")}: ${stageLabel(stage)}`);
	} catch (cause) {
		error.value = toErrorMessage(cause);
	} finally {
		advancingId.value = "";
...
function stageLabel(stage?: SalesOpportunityStage | null) { return stage ? valueLabel(stage) : t("pharma.salesOpportunity.unknownStage"); }
...
<template>
	<PageShell :title="t('pharma.salesOpportunity.salesOpportunities')" :loading="loading" :error="error" :forbidden="!canRead" :forbidden-title="t('pharma.salesOpportunity.noOpportunityAccess')" :forbidden-description="t('pharma.salesOpportunity.thisPageRequiresPharmaOaSalesOpportunityReadPermission')">
		<template #actions>
			<el-input v-model="keyword" clearable :placeholder="t('pharma.salesOpportunity.searchOpportunityOrCustomer')" class="filter-control" @keyup.enter="refresh" />
			<el-select v-model="stageFilter" clearable :placeholder="t('pharma.salesOpportunity.allStages')" class="filter-control" @change="refresh"><el-option v-for="stage in stages" :key="stage" :label="stageLabel(stage)" :value="stage" /></el-select>
			<el-tooltip :content="t('pharma.salesOpportunity.refresh')"><el-button circle :icon="RefreshCw" :aria-label="t('pharma.salesOpportunity.refreshOpportunities')" :loading="loading" @click="refresh" /></el-tooltip>
			<el-button type="primary" :icon="Plus" :disabled="!canCreate || customers.length === 0 || products.length === 0" @click="openCreate">{{ t("pharma.salesOpportunity.newOpportunity") }}</el-button>
		</template>

		<section class="funnel-band" :aria-label="t('pharma.salesOpportunity.salesFunnelStatistics')">
			<div class="funnel-total"><span>{{ t("pharma.salesOpportunity.pipeline") }}</span><strong>{{ formatMoney(statistics.expectedAmountCents) }}</strong><small>{{ statistics.openCount }} {{ t("pharma.salesOpportunity.open") }} {{ statistics.wonCount }} {{ t("pharma.salesOpportunity.won") }}</small></div>
		...
		<DataTable :rows="rows" :columns="columns" row-key="id" :loading="loading" :error="error" :empty-text="t('pharma.salesOpportunity.noSalesOpportunitiesMatchTheCurrentFilters')">
			...
			<template #actions="{ row }">
				<el-tooltip :content="t('pharma.salesOpportunity.viewDetails')"><el-button circle :icon="Eye" :aria-label="t('pharma.salesOpportunity.viewOpportunity')" @click="openDetail(row as SalesOpportunity)" /></el-tooltip>
				<el-tooltip v-if="!isTerminal(row.stage as SalesOpportunityStage)" :content="t('pharma.salesOpportunity.edit')"><el-button circle :icon="Pencil" :aria-label="t('pharma.salesOpportunity.editOpportunity')" :disabled="!canUpdate" @click="openEdit(row as SalesOpportunity)" /></el-tooltip>
				<el-tooltip v-if="nextStage(row.stage as SalesOpportunityStage)" :content="`Advance to ${stageLabel(nextStage(row.stage as SalesOpportunityStage)!)}`"><el-button circle type="primary" :icon="ArrowRight" :aria-label="t('pharma.salesOpportunity.advanceOpportunity')" :loading="advancingId === row.id" :disabled="!canAdvance" @click="advance(row as SalesOpportunity)" /></el-tooltip>
				<el-tooltip v-if="!isTerminal(row.stage as SalesOpportunityStage)" :content="t('pharma.salesOpportunity.closeAsLost')"><el-button circle type="danger" :icon="XCircle" :aria-label="t('pharma.salesOpportunity.closeOpportunityAsLost')" :loading="advancingId === row.id" :disabled="!canAdvance" @click="lose(row as SalesOpportunity)" /></el-tooltip>
			</template>
		</DataTable>

		<DetailDrawer v-model="editOpen" :title="editingId ? t('pharma.salesOpportunity.editOpportunity') : t('pharma.salesOpportunity.newOpportunity')" size="52%">
			<el-form label-position="top"><el-form-item :label="t('pharma.salesOpportunity.title')" required><el-input v-model="form.title" maxlength="120" /></el-form-item><div class="form-grid"><el-form-item :label="t('pharma.salesOpportunity.customer')" required><el-select v-model="form.customerId" filterable :disabled="Boolean(editingId)"><el-option v-for="customer in customers" :key="customer.id" :label="`${customer.code} · ${customer.name}`" :value="customer.id" /></el-select></el-form-item><el-form-item :label="t('pharma.salesOpportunity.expectedAmount')" required><el-input-number v-model="form.expectedAmount" :min="0" :precision="2" :step="100" controls-position="right" /></el-form-item></div><div class="form-grid"><el-form-item :label="t('pharma.salesOpportunity.products')" required><el-select v-model="form.productIds" multiple filterable collapse-tags><el-option v-for="product in products" :key="product.id" :label="`${product.code} · ${product.name}`" :value="product.id" /></el-select></el-form-item><el-form-item :label="t('pharma.salesOpportunity.estimatedClose')" required><el-date-picker v-model="form.estimatedCloseDate" type="date" /></el-form-item></div></el-form>
			<template #footer><el-button :disabled="saving" @click="editOpen = false">{{ t("pharma.salesOpportunity.close") }}</el-button><el-button type="primary" :loading="saving" :disabled="!form.title.trim() || !form.customerId || form.productIds.length === 0 || form.expectedAmount <= 0 || !form.estimatedCloseDate" @click="save">{{ t("pharma.salesOpportunity.saveOpportunity") }}</el-button></template>
		</DetailDrawer>

		<DetailDrawer v-model="detailOpen" :title="selected?.title || t('pharma.salesOpportunity.detailTitle')" size="50%"><template v-if="selected"><div class="detail-status"><el-tag :type="stageType(selected.stage)">{{ stageLabel(selected.stage) }}</el-tag><strong>{{ formatMoney(selected.expectedAmountCents) }}</strong></div><dl class="detail-grid"><div><dt>{{ t("pharma.salesOpportunity.customer") }}</dt><dd>{{ selected.customerCode }} · {{ selected.customerName }}</dd></div><div><dt>{{ t("pharma.salesOpportunity.owner") }}</dt><dd>{{ selected.ownerId }}</dd></div><div><dt>{{ t("pharma.salesOpportunity.products") }}</dt><dd>{{ selected.products.map((product) => product.productName).join(', ') }}</dd></div><div><dt>{{ t("pharma.salesOpportunity.estimatedClose") }}</dt><dd>{{ formatDate(selected.estimatedCloseDate) }}</dd></div><div v-if="selected.lostReason" class="wide"><dt>{{ t("pharma.salesOpportunity.lostReason") }}</dt><dd>{{ selected.lostReason }}</dd></div></dl><section class="timeline"><h3><TrendingUp :size="18" /> {{ t("pharma.salesOpportunity.stageHistory") }}</h3><el-timeline><el-timeline-item v-for="change in [...selected.stageHistory].reverse()" :key="`${change.to}-${change.changedAt}`" :timestamp="formatDateTime(change.changedAt)" placement="top"><strong>{{ stageLabel(change.to) }}</strong><p>{{ change.note || `${t('pharma.salesOpportunity.changedBy')}${change.changedBy}` }}</p></el-timeline-item></el-timeline></section></template><template #footer><el-button @click="detailOpen = false">{{ t("pharma.salesOpportunity.close") }}</el-button><el-button v-if="selected && nextStage(selected.stage)" type="primary" :icon="ArrowRight" :disabled="!canAdvance" @click="advance(selected)">{{ t("pharma.salesOpportunity.advance") }}</el-button></template></DetailDrawer>
	</PageShell>
</template>
