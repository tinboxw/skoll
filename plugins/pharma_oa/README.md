...
> 当前定位：可演示、可测试、可二次开发的行业样板，不是经过法规验证的生产级 GSP/GMP 系统。
...
- 当前医药 OA 业务仓储为进程内实现。重启后需重新应用演示数据；报表文件使用现有私有对象存储。
...
- 现有部分医药 OA 集成页仍包含英文业务文案；这是已知样板边界，不代表完整页面本地化已经验收。
...
| 演示数据消失 | 当前仓储为进程内实现；重启后重新调用 demo seed |
