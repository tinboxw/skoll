(function () {
	...
	var state = { items: [], reminders: [], editing: null, target: null, loading: false, locale: "zh-CN", module: "employee", statusAction: "disable" };
	...
	async function load() {
		...
		try {
			...
			var collection = state.module === "employee" ? "/employees" : partyCollections[state.module];
			var results = state.module === "employee" ? await Promise.all([request(collection + query({ keyword: keyword, status: status, limit: 200 })), request("/employees/qualification-reminders?days=30")]) : [await request(collection + query({ keyword: keyword, status: status, limit: 200 })), { items: [] }];
	...
	function render() {
		if (state.module !== "employee") { renderParties(); return; }
	...
	document.querySelector("#create-button").addEventListener("click", function () { state.module === "employee" ? openEmployee(null) : openParty(null); });
	document.querySelector("#empty-create-button").addEventListener("click", function () { state.module === "employee" ? openEmployee(null) : openParty(null); });
	...
	document.querySelectorAll("[data-module]").forEach(function(button){button.addEventListener("click",function(){if(state.module===button.dataset.module)return; state.module=button.dataset.module; state.items=[]; state.reminders=[]; document.querySelector("#keyword-input").value=""; applyModule(); load();});});
