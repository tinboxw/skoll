...
| 250 | M7-06-02 | M7-06 | `skoll-community-governance` | Issue/PR 模板 | `.github` | bug、feature、task、PR 模板包含复现、验收、无兼容规则确认 | 文档审阅 | Done |
| 251 | M7-06-03 | M7-06 | `skoll-devops-deployment` | Release checklist | docs | 版本号、迁移、镜像、OpenAPI、示例、质量门禁清单完整 | 文档审阅 | Done |
| 252 | M7-06-04 | M7-06 | `skoll-refactor-governance` | 发布候选最终巡检 | docs/refactor | work_items、task_board、acceptance_log、git log、质量门禁一致 | `git log --oneline -30`; 文档审阅 | Done |
