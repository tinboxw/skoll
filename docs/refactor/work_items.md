...
| 205 | M4-06-02 | M4-06 | `skoll-permission-rbac` | 用户列表数据范围过滤 | service/store | 不同角色返回不同用户列表，super_admin 绕过明确 | `go test ./internal/service/user/... ./internal/service/rbac/...` | Done |
| 206 | M4-07-01 | M4-07 | `skoll-vue-frontend` | 组织管理 UI | Vue page | 部门树、岗位、用户归属编辑可用 | `cd web && npm run build` | Done |
| 207 | M4-07-02 | M4-07 | `skoll-frontend-testing-refactor` | 数据范围验收清单 | docs/smoke | admin、部门管理员、普通用户三类结果可复查 | 文档审阅; `cd web && npm run build` | Done |
...
| 213 | M5-04-01 | M5-04 | `skoll-code-generator` | 后端 domain/store 生成 | templates | domain、migration、store、repository 输出可编译 | `go test ./...` | Done |
| 214 | M5-04-02 | M5-04 | `skoll-code-generator` | 后端 service/handler/OpenAPI 生成 | templates | CRUD API、权限、审计、OpenAPI 同步 | `go test ./...` | Done |
| 215 | M5-05-01 | M5-05 | `skoll-code-generator` | 前端 API/store 生成 | templates | API client、Pinia store、错误状态生成 | `cd web && npm run typecheck` | Done |
| 216 | M5-05-02 | M5-05 | `skoll-code-generator` | 前端 list/form 页面生成 | templates | 列表、筛选、表单、状态组件、权限按钮生成 | `cd web && npm run build` | Done |
...
| 222 | M6-01-02 | M6-01 | `skoll-plugin-marketplace-refactor` | index 校验器 | service/tests | 无效索引、重复插件、版本冲突能报错 | `go test ./internal/plugin/...` | Done |
| 223 | M6-02-01 | M6-02 | `skoll-plugin-platform` | 本地市场 service/API | service/API | 本地插件、可安装包、签名、风险可展示 | `go test ./internal/plugin/... ./internal/handler/http/v1/plugin/...` | Done |
| 224 | M6-02-02 | M6-02 | `skoll-vue-frontend` | 市场列表 UI | Vue page | 搜索、筛选、风险、签名、版本、安装入口完整 | `cd web && npm run build` | Done |
| 225 | M6-03-01 | M6-03 | `skoll-plugin-marketplace-refactor` | 远程 index adapter | service | 远程失败不影响本地插件，错误可见 | `go test ./internal/plugin/...` | Done |
| 226 | M6-03-02 | M6-03 | `skoll-plugin-marketplace-refactor` | 安装预检 service | service | 权限 diff、菜单 diff、配置、资源、迁移、签名、风险完整 | `go test ./internal/plugin/...` | Done |
| 227 | M6-03-03 | M6-03 | `skoll-web-ui-design` | 安装预检 UI | Vue page | 高风险动作确认、阻断原因、审计线索可读 | `cd web && npm run build` | Done |
| 228 | M6-04-01 | M6-04 | `skoll-plugin-platform` | 插件迁移 hook | plugin/service | install/upgrade/downgrade/uninstall 状态可追踪 | `go test ./internal/plugin/...` | Done |
...
| 241 | M7-04-02 | M7-04 | `skoll-docs-writer` | 运维手册：配置与部署 | docs | 配置项、环境变量、Docker/Compose、服务启动和停止路径明确 | 文档审阅 | Done |
| 242 | M7-04-03 | M7-04 | `skoll-docs-writer` | 运维手册：备份、日志与故障处理 | docs | 数据备份/恢复、日志定位、常见故障恢复步骤可执行 | 文档审阅 | Done |
...
| 247 | M7-05-04 | M7-05 | `skoll-plugin-platform` | 示例插件 manifest 与资产 | examples/plugins | demo plugin manifest、权限、菜单、配置 schema、签名/风险字段完整 | manifest schema review | Done |
| 248 | M7-05-05 | M7-05 | `skoll-testing-automation` | 示例插件生命周期验收 | examples/plugins/tests | demo plugin 可安装、启用、禁用、升级、回滚；审计事件可查 | `go test ./...`; `cd web && npm run build` | Done |
| 249 | M7-06-01 | M7-06 | `skoll-community-governance` | 开源贡献指南 | docs/.github | CONTRIBUTING、SECURITY、行为准则或维护者说明完整 | 文档审阅 | Done |
| 250 | M7-06-02 | M7-06 | `skoll-community-governance` | Issue/PR 模板 | `.github` | bug、feature、task、PR 模板包含复现、验收、无兼容规则确认 | 文档审阅 | Todo |
| 251 | M7-06-03 | M7-06 | `skoll-devops-deployment` | Release checklist | docs | 版本号、迁移、镜像、OpenAPI、示例、质量门禁清单完整 | 文档审阅 | Todo |
| 252 | M7-06-04 | M7-06 | `skoll-refactor-governance` | 发布候选最终巡检 | docs/refactor | work_items、task_board、acceptance_log、git log、质量门禁一致 | `git log --oneline -30`; 文档审阅 | Todo |
