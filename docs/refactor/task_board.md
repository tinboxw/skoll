...
|---:|---|---|---|---|---|---|---|
| 1 | M0-01 | `skoll-refactor-governance` | 固化架构模型 | 架构决策文档 | 明确 Modular Monolith + Clean/Hexagonal + Tactical DDD；明确不做兼容方案 | 文档审阅 | Todo |
| 2 | M0-02 | `skoll-refactor-governance` | 建立任务验收模板 | `acceptance_log.md` | 模板包含改动文件、命令结果、人工验收、提交信息 | 文档审阅 | Todo |
| 3 | M0-03 | `skoll-quality-gate` | 建立当前质量基线 | 当前测试/构建/覆盖率记录 | `go test ./...`、覆盖率、前端 build/typecheck 有真实结果 | `go test ./...`; `go test ./... -coverprofile=coverage.out`; `cd web && npm run build` | Todo |
...
| 顺序 | ID | Skill | 任务 | 交付物 | 验收要求 | 验证命令 | 状态 |
|---:|---|---|---|---|---|---|---|
| 22 | M3-01 | `skoll-file-storage-refactor` | 定义 FileObject 与 ObjectStore port | domain + port | 字段覆盖 key、name、size、mime、hash、owner、visibility、storage、status | `go test ./internal/domain/file/...` | Todo |
