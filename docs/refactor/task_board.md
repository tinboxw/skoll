...
| 55 | M7-04 | `skoll-docs-writer` | 快速开始和运维手册 | docs | 新环境 30 分钟内启动、登录、安装插件、生成模块；运维配置、备份、日志和故障处理可复现 | 手工复现 | Done |
| 56 | M7-05 | `skoll-open-source-framework` | 示例模块与示例插件 | examples/docs | 至少一个生成模块和一个插件示例可运行，且覆盖前后端和生命周期验收 | `go test ./...`; `cd web && npm run build` | Done |
| 57 | M7-06 | `skoll-community-governance` / `skoll-devops-deployment` | 开源协作与发布检查清单 | docs/.github | 贡献模板、Issue/PR 模板、Release checklist 和最终巡检完整 | 文档审阅; `git log --oneline -30` | Done |
