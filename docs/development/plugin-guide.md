...
> Go 插件宿主能力见 [插件 SDK 当前契约](plugin-sdk-reference.md)；结构化关系数据模型见 [插件数据存储契约](plugin-datastore-contract.md)；独立后端启动规则见 [插件受管进程契约](plugin-process-contract.md)；业务 API、权限和聚合 OpenAPI 的当前格式见 [插件 API 与 OpenAPI 契约](plugin-api-contract.md)，英文版见 [Plugin API and OpenAPI Contract](plugin-api-contract.en.md)。
