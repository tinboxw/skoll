...
实例决策统一通过 repository 的原子更新边界执行：SQL 模式在事务内锁定实例父记录、重建聚合、判断动作历史并提交；内存模式在同一互斥区完成等价操作。审批、驳回、撤回、转交和抄送以动作类型、任务、执行人和目标组成语义指纹，同一决策重试返回首次提交结果，不重复追加任务或历史。数据库锁冲突、死锁和序列化失败只重试无外部副作用的领域变更回调，约束冲突不会重试。
