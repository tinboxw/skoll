...
func TestPharmaOAPluginManifestCoversCurrentIndustryBoundary(t *testing.T) {
	...
	if info.Version != "0.3.0" || info.APIVersion != "v1" || info.MigrationVersion != info.Version {
	...
	if info.DataManifest == nil || info.DataManifest.Namespace != info.ID || info.DataManifest.MigrationVersion != info.Version || info.DataManifest.MigrationDirectory != "migrations" || info.DataManifest.UninstallPolicy != DataUninstallDrop || info.DataManifest.RollbackPolicy != DataRollbackAutomatic || len(info.DataManifest.Tables) != 3 {
