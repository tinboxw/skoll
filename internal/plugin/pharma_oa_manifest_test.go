...
func TestPharmaOAPluginManifestCoversCurrentIndustryBoundary(t *testing.T) {
	...
	if info.Version != "0.5.0" || info.APIVersion != "v1" || info.MigrationVersion != info.Version {
	...
	if info.DataManifest == nil || info.DataManifest.Namespace != info.ID || info.DataManifest.MigrationVersion != info.Version || info.DataManifest.MigrationDirectory != "migrations" || info.DataManifest.UninstallPolicy != DataUninstallDrop || info.DataManifest.RollbackPolicy != DataRollbackAutomatic || len(info.DataManifest.Tables) != 6 {
	...
	for _, required := range []string{"pharma_oa.foundation.read", "pharma_oa.menu.read", "pharma_oa.seed.read", "pharma_oa.employee.read", "pharma_oa.product.read", "pharma_oa.product.enable", "pharma_oa.category.read", "pharma_oa.unit.read", "pharma_oa.manufacturer.read", "pharma_oa.qualification.read"} {
