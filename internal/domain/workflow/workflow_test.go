...
func TestWorkflowStartApproveRejectAndWithdraw(t *testing.T) {
	...
	if err := instance.Approve(*def, taskID, Actor{ID: "manager-1"}, "ok", nil, now.Add(time.Minute)); err != nil {
	...
	if err := instance.Reject(*def, taskID, Actor{ID: "manager-1"}, "late", nil, now.Add(2*time.Minute)); err == nil {
		t.Fatal("expected terminal instance to reject repeated action")
	}

	rejected := mustStartInstance(t, *def, "wf-2", now)
	if err := rejected.Reject(*def, rejected.Tasks[0].ID, Actor{ID: "manager-1"}, "no", nil, now.Add(time.Minute)); err != nil {
...
func TestWorkflowDelegateAndCopy(t *testing.T) {
	...
	if err := instance.Approve(*def, instance.Tasks[2].ID, Actor{ID: "manager-2"}, "ok", nil, now.Add(3*time.Minute)); err != nil {
...
func TestWorkflowRejectsInvalidActorAndDefinitionStates(t *testing.T) {
	...
	if err := instance.Approve(*def, instance.Tasks[0].ID, Actor{ID: "other-manager"}, "ok", nil, now.Add(time.Minute)); err == nil {
