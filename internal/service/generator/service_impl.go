...
func buildCandidates(spec domaingenerator.GeneratorSpec, migrationTimestamp string) []fileCandidate {
	...
	if !spec.Plugin.Enabled {
		return append(candidates,
			...
			candidate("backend.domain.test", fmt.Sprintf("internal/domain/%s/entity_test.go", module), spec),
			...
			candidate("backend.store.memory.test", fmt.Sprintf("internal/store/memory/%s_store_test.go", module), spec),
			...
			candidate("backend.store.gorm.test", fmt.Sprintf("internal/store/sql/gormrepo/%s_store_test.go", module), spec),
			...
			candidate("backend.service.test", fmt.Sprintf("internal/service/%s/service_impl_test.go", module), spec),
			candidate("backend.handler", fmt.Sprintf("internal/handler/http/v1/%s/handler.go", module), spec),
			candidate("backend.handler.test", fmt.Sprintf("internal/handler/http/v1/%s/handler_test.go", module), spec),
			candidate("backend.router", fmt.Sprintf("internal/handler/http/generated_%s_routes.go", module), spec),
	...
	candidates = append(candidates,
		...
		candidate("plugin.contract", pluginRoot+"/contract.json", spec),
		candidate("plugin.datastore.schema", pluginRoot+"/datastore.yaml", spec),
		candidate("plugin.go.mod", pluginRoot+"/go.mod", spec),
		candidate("plugin.backend.contract", pluginRoot+"/backend/contract.go", spec),
		candidate("plugin.backend.server", pluginRoot+"/backend/main.go", spec),
		...
		candidate("plugin.frontend.contract", pluginRoot+"/web/src/contract.ts", spec),
		...
		candidate("plugin.frontend.package", pluginRoot+"/web/package.json", spec),
		candidate("plugin.frontend.tsconfig", pluginRoot+"/web/tsconfig.json", spec),
		candidate("plugin.frontend.vite", pluginRoot+"/web/vite.config.ts", spec),
		candidate("plugin.frontend.index", pluginRoot+"/web/index.html", spec),
		candidate("plugin.frontend.main", pluginRoot+"/web/src/main.ts", spec),
		candidate("plugin.frontend.styles", pluginRoot+"/web/src/styles.css", spec),
		candidate("plugin.frontend.api-support", pluginRoot+"/web/src/utils/api.ts", spec),
		candidate("plugin.frontend.common-support", pluginRoot+"/web/src/utils/common.ts", spec),
		candidate("plugin.frontend.host-support", pluginRoot+"/web/src/skoll-host.ts", spec),
...
func allowedOutputPath(path string) bool {
	...
	allowedPrefixes := []string{
		...
		"internal/handler/http/generated_",
