...
var schemaBaseline = []TableSpec{
	table("pharma_oa_employees", "master.employee", []string{"code", "name", "department_id", "position_id", "status", "joined_at", "left_at", "certificates_json"}, []IndexSpec{index("uk_pharma_employees_code", true, "code"), index("idx_pharma_employees_org_status", false, "department_id", "status"), index("idx_pharma_employees_name", false, "name")}, "master"),
	table("pharma_oa_products", "master.product", []string{"code", "name", "specification", "dosage_form", "manufacturer", "approval_number", "status", "temperature_json"}, []IndexSpec{index("uk_pharma_products_code", true, "code"), index("uk_pharma_products_approval", true, "approval_number"), index("idx_pharma_products_status_name", false, "status", "name")}, "master"),
	table("pharma_oa_suppliers", "master.supplier", []string{"code", "name", "status", "contacts_json", "qualifications_json", "attachments_json"}, []IndexSpec{index("uk_pharma_suppliers_code", true, "code"), index("idx_pharma_suppliers_status_name", false, "status", "name")}, "master"),
	table("pharma_oa_customers", "master.customer", []string{"code", "name", "status", "owner_id", "organization_id", "contacts_json", "qualifications_json", "attachments_json"}, []IndexSpec{index("uk_pharma_customers_code", true, "code"), index("idx_pharma_customers_scope_status", false, "organization_id", "owner_id", "status"), index("idx_pharma_customers_name", false, "name")}, "master"),
	table("pharma_oa_warehouses", "master.warehouse", []string{"code", "name", "status", "temperature_json", "areas_json"}, []IndexSpec{index("uk_pharma_warehouses_code", true, "code"), index("idx_pharma_warehouses_status_name", false, "status", "name")}, "master"),
	...
	table("pharma_oa_purchase_requests", "purchase.request", []string{"number", "supplier_id", "requester_id", "approver_id", "reason", "total_amount", "status", "workflow_instance_id", "purchase_order_id"}, []IndexSpec{index("uk_pharma_purchase_requests_number", true, "number"), index("idx_pharma_purchase_requests_status_approver", false, "status", "approver_id"), index("idx_pharma_purchase_requests_supplier", false, "supplier_id")}, "purchase"),
	table("pharma_oa_purchase_request_lines", "purchase.request", []string{"request_id", "line_no", "product_id", "quantity", "unit_price", "amount"}, []IndexSpec{index("uk_pharma_purchase_request_line", true, "request_id", "line_no"), index("idx_pharma_purchase_request_product", false, "product_id")}, "purchase"),
	table("pharma_oa_purchase_orders", "purchase.order", []string{"number", "purchase_request_id", "supplier_id", "total_amount", "status", "approved_by", "approved_at"}, []IndexSpec{index("uk_pharma_purchase_orders_number", true, "number"), index("uk_pharma_purchase_orders_request", true, "purchase_request_id"), index("idx_pharma_purchase_orders_supplier_status", false, "supplier_id", "status")}, "purchase"),
	table("pharma_oa_purchase_inbounds", "purchase.inbound", []string{"number", "purchase_order_id", "warehouse_id", "status", "received_by", "received_at", "attachments_json", "idempotency_key"}, []IndexSpec{index("uk_pharma_inbounds_number", true, "number"), index("uk_pharma_inbounds_idempotency", true, "idempotency_key"), index("idx_pharma_inbounds_order", false, "purchase_order_id")}, "inventory"),
	table("pharma_oa_sales_orders", "sales.order", []string{"number", "customer_id", "total_amount", "status", "organization_id"}, []IndexSpec{index("uk_pharma_sales_orders_number", true, "number"), index("idx_pharma_sales_orders_customer_status", false, "customer_id", "status"), index("idx_pharma_sales_orders_scope", false, "organization_id", "created_at")}, "sales"),
	table("pharma_oa_sales_outbounds", "sales.outbound", []string{"number", "sales_order_id", "customer_id", "warehouse_id", "area_id", "location_id", "status", "shipped_by", "shipped_at", "idempotency_key"}, []IndexSpec{index("uk_pharma_outbounds_number", true, "number"), index("uk_pharma_outbounds_idempotency", true, "idempotency_key"), index("idx_pharma_outbounds_order", false, "sales_order_id")}, "inventory"),
	table("pharma_oa_stocktakes", "inventory.stocktake", []string{"number", "warehouse_id", "status", "workflow_instance_id", "lines_json"}, []IndexSpec{index("uk_pharma_stocktakes_number", true, "number"), index("idx_pharma_stocktakes_warehouse_status", false, "warehouse_id", "status")}, "inventory"),
	table("pharma_oa_transfers", "inventory.transfer", []string{"number", "source_warehouse_id", "target_warehouse_id", "status", "lines_json", "idempotency_key"}, []IndexSpec{index("uk_pharma_transfers_number", true, "number"), index("uk_pharma_transfers_idempotency", true, "idempotency_key"), index("idx_pharma_transfers_status", false, "status")}, "inventory"),
	...
	table("pharma_oa_quality_complaints", "compliance.complaint", []string{"number", "title", "customer_id", "product_id", "batch_id", "status", "workflow_instance_id", "attachments_json"}, []IndexSpec{index("uk_pharma_complaints_number", true, "number"), index("idx_pharma_complaints_status_product", false, "status", "product_id"), index("idx_pharma_complaints_batch", false, "batch_id")}, "compliance"),
	table("pharma_oa_drug_recalls", "compliance.recall", []string{"number", "title", "product_id", "batch_id", "source_complaint_id", "status", "tasks_json"}, []IndexSpec{index("uk_pharma_recalls_number", true, "number"), index("idx_pharma_recalls_batch_status", false, "batch_id", "status")}, "compliance"),
	...
	table("pharma_oa_customer_follow_ups", "crm.follow_up", []string{"customer_id", "owner_id", "organization_id", "status", "planned_at", "completed_at", "attachments_json"}, []IndexSpec{index("idx_pharma_followups_scope_status", false, "organization_id", "owner_id", "status"), index("idx_pharma_followups_customer_time", false, "customer_id", "planned_at")}, "crm"),
	table("pharma_oa_sales_opportunities", "crm.opportunity", []string{"number", "customer_id", "owner_id", "organization_id", "stage", "expected_amount", "expected_close_at", "products_json", "stage_history_json"}, []IndexSpec{index("uk_pharma_opportunities_number", true, "number"), index("idx_pharma_opportunities_scope_stage", false, "organization_id", "owner_id", "stage"), index("idx_pharma_opportunities_customer", false, "customer_id")}, "crm"),
	table("pharma_oa_payment_plans", "finance.payment", []string{"sales_order_id", "customer_id", "amount", "received_amount", "status", "due_at", "receipts_json"}, []IndexSpec{index("uk_pharma_payment_plan_order", true, "sales_order_id"), index("idx_pharma_payment_plan_due_status", false, "status", "due_at")}, "finance"),
	table("pharma_oa_invoice_records", "finance.invoice", []string{"number", "sales_order_id", "customer_id", "amount", "status", "issued_at", "voided_at", "attachments_json"}, []IndexSpec{index("uk_pharma_invoices_number", true, "number"), index("idx_pharma_invoices_order_status", false, "sales_order_id", "status")}, "finance"),
	table("pharma_oa_inventory_alert_jobs", "jobs.inventory_alert", []string{"status", "attempt", "idempotency_key", "started_at", "finished_at", "error", "logs_json"}, []IndexSpec{index("uk_pharma_alert_jobs_idempotency", true, "idempotency_key"), index("idx_pharma_alert_jobs_status_created", false, "status", "created_at")}, "jobs"),
	table("pharma_oa_report_export_jobs", "jobs.report_export", []string{"report_type", "status", "attempt", "idempotency_key", "file_id", "started_at", "finished_at", "error"}, []IndexSpec{index("uk_pharma_export_jobs_idempotency", true, "idempotency_key"), index("idx_pharma_export_jobs_status_created", false, "status", "created_at")}, "jobs"),
}
