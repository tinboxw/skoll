...
func TestSystemStoreImplementsSystemRepository(t *testing.T) {
...
func TestSystemStoreDictionaryTypeAndItemContract(t *testing.T) {
	...
	dictType := mustMemoryDictionaryType(t, "type-1", " System.Locale ", 20)
	...
	secondType := mustMemoryDictionaryType(t, "type-2", "system.status", 10)
	...
	firstItem := mustMemoryDictionaryItem(t, "item-1", "system.locale", "English", "en", 20)
	secondItem := mustMemoryDictionaryItem(t, "item-2", "system.locale", "Chinese", "zh-CN", 10)
...
func TestSystemStoreDictionaryRejectsDuplicatesAndMissingType(t *testing.T) {
	ctx := context.Background()
	store := NewSystemStore()
	dictType := mustMemoryDictionaryType(t, "type-1", "system.locale", 10)
	if err := store.SaveDictionaryType(ctx, dictType); err != nil {
		t.Fatalf("SaveDictionaryType() error = %v", err)
	}
	dupType := mustMemoryDictionaryType(t, "type-2", "SYSTEM.LOCALE", 20)
	if err := store.SaveDictionaryType(ctx, dupType); err == nil || !strings.Contains(err.Error(), "already exists") {
		t.Fatalf("expected duplicate type error, got %v", err)
	}

	firstItem := mustMemoryDictionaryItem(t, "item-1", "system.locale", "English", "en", 10)
	if err := store.SaveDictionaryItem(ctx, firstItem); err != nil {
		t.Fatalf("SaveDictionaryItem() error = %v", err)
	}
	dupItem := mustMemoryDictionaryItem(t, "item-2", "system.locale", "English again", "en", 20)
	if err := store.SaveDictionaryItem(ctx, dupItem); err == nil || !strings.Contains(err.Error(), "already exists") {
		t.Fatalf("expected duplicate item error, got %v", err)
	}
	missingTypeItem := mustMemoryDictionaryItem(t, "item-3", "system.missing", "Missing", "missing", 30)
	if err := store.SaveDictionaryItem(ctx, missingTypeItem); err == nil || !strings.Contains(err.Error(), "does not exist") {
		t.Fatalf("expected missing type error, got %v", err)
	}
}

func mustMemoryDictionaryType(t *testing.T, id, code string, sort int) *domainsystem.DictionaryType {
	t.Helper()
...
func mustMemoryDictionaryItem(t *testing.T, id, typeCode, label, value string, sort int) *domainsystem.DictionaryItem {
	t.Helper()
	now := time.Date(2026, 6, 22, 12, 0, 0, 0, time.UTC)
