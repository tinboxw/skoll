...
func TestOrganizationStoreDepartmentPositionAndAssignmentContract(t *testing.T) {
	...
	root := mustGormDepartment(t, "dept-root", "", "root", 0)
	sales := mustGormDepartment(t, "dept-sales", "dept-root", "sales", 20)
	ops := mustGormDepartment(t, "dept-ops", "dept-root", "ops", 10)
	...
	manager := mustGormPosition(t, "pos-manager", "manager", 20)
	associate := mustGormPosition(t, "pos-associate", "associate", 10)
	...
	assignment := mustGormAssignment(t, "user-1", "dept-sales", "pos-manager")
...
func TestOrganizationStoreRejectsInvalidReferences(t *testing.T) {
	...
	if err := store.SaveDepartment(ctx, mustGormDepartment(t, "dept-child", "missing", "child", 10)); err == nil || !strings.Contains(err.Error(), "parent") {
		t.Fatalf("expected missing parent error, got %v", err)
	}
	if err := store.SaveDepartment(ctx, mustGormDepartment(t, "dept-root", "", "root", 10)); err != nil {
		t.Fatalf("SaveDepartment(root) error = %v", err)
	}
	if err := store.SaveDepartment(ctx, mustGormDepartment(t, "dept-root-2", "", "ROOT", 20)); err == nil {
		t.Fatal("expected duplicate code error")
	}
	if err := store.SaveUserAssignment(ctx, mustGormAssignment(t, "user-1", "missing", "")); err == nil || !strings.Contains(err.Error(), "department") {
		t.Fatalf("expected missing department error, got %v", err)
	}
	if err := store.SaveUserAssignment(ctx, mustGormAssignment(t, "user-1", "dept-root", "missing")); err == nil || !strings.Contains(err.Error(), "position") {
		t.Fatalf("expected missing position error, got %v", err)
	}
}

func mustGormDepartment(t *testing.T, id, parentID, code string, sortValue int) *domainorg.Department {
	t.Helper()
...
func mustGormPosition(t *testing.T, id, code string, sortValue int) *domainorg.Position {
	t.Helper()
	now := time.Date(2026, 6, 29, 12, 0, 0, 0, time.UTC)
...
func mustGormAssignment(t *testing.T, userID, departmentID, positionID string) *domainorg.UserAssignment {
	t.Helper()
	now := time.Date(2026, 6, 29, 12, 0, 0, 0, time.UTC)
