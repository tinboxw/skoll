...
type PluginModel struct {
	...
	SystemBuiltin        bool
	HostCapabilitiesJSON string `gorm:"type:text"`
...
func PluginModelFromInfo(info plugin.Info) PluginModel {
	...
	hostCapabilities, _ := json.Marshal(info.HostCapabilities)
	...
	return PluginModel{
		...
		HostCapabilitiesJSON: string(hostCapabilities),
...
func (m PluginModel) ToInfo() plugin.Info {
	...
	if strings.TrimSpace(m.HostCapabilitiesJSON) != "" {
		_ = json.Unmarshal([]byte(m.HostCapabilitiesJSON), &hostCapabilities)
	}
