...
func TestEmployeeHandlerCreateListLeaveAndReminders(t *testing.T) {
	...
	createResp := performRequest(mux, http.MethodPost, "/v1/plugins/pharma_oa/api/employees", createBody)
	...
	listResp := performRequest(mux, http.MethodGet, "/v1/plugins/pharma_oa/api/employees?keyword=alice", nil)
	...
	pageResp := performRequest(mux, http.MethodGet, "/v1/plugins/pharma_oa/api/employees?limit=1", nil)
	...
	invalidCursor := performRequest(mux, http.MethodGet, "/v1/plugins/pharma_oa/api/employees?cursor=not-valid-*", nil)
	...
	reminderResp := performRequest(mux, http.MethodGet, "/v1/plugins/pharma_oa/api/employees/qualification-reminders?days=60", nil)
	...
	leaveResp := performRequest(mux, http.MethodPost, "/v1/plugins/pharma_oa/api/employees/"+createPayload.Data.Item.ID+"/leave", []byte(`{"reason":"resigned","actorId":"admin"}`))
...
func TestActorIDFromRequestPrefersJWTSubject(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/v1/plugins/pharma_oa/api/employees", nil)
...
func TestPharmaListCursorRoundTrip(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/v1/plugins/pharma_oa/api/employees?cursor=NTA&limit=25", nil)
...
func TestEmployeeListSerializesEmptyCertificatesAsArray(t *testing.T) {
	...
	mux := http.NewServeMux()
	RegisterEmployeeRoutes(mux, service)
	created := performRequest(mux, http.MethodPost, "/v1/plugins/pharma_oa/api/employees", []byte(`{"code":"EMP-EMPTY","name":"Empty certificates","departmentId":"quality","positionId":"qa","certificates":[]}`))
	if created.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", created.Code, created.Body.String())
	}
	listed := performRequest(mux, http.MethodGet, "/v1/plugins/pharma_oa/api/employees?keyword=EMP-EMPTY", nil)
