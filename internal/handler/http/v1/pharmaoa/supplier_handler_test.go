...
func TestSupplierHandlerCreateReminderAndPurchaseBlock(t *testing.T) {
	...
	createResp := performRequest(mux, http.MethodPost, "/v1/plugins/pharma_oa/api/suppliers", []byte(fmt.Sprintf(`{
		"code":"SUP001",
	...
	reminderResp := performRequest(mux, http.MethodGet, "/v1/plugins/pharma_oa/api/suppliers/qualification-reminders?days=60", nil)
	...
	eligibleResp := performRequest(mux, http.MethodGet, "/v1/plugins/pharma_oa/api/suppliers/"+createPayload.Data.Item.ID+"/purchase-eligibility", nil)
	...
	disableResp := performRequest(mux, http.MethodPost, "/v1/plugins/pharma_oa/api/suppliers/"+createPayload.Data.Item.ID+"/disable", []byte(`{"reason":"blacklisted","actorId":"admin"}`))
	...
	blockResp := performRequest(mux, http.MethodGet, "/v1/plugins/pharma_oa/api/suppliers/"+createPayload.Data.Item.ID+"/purchase-eligibility", nil)
...
func TestSupplierHandlerRejectsUnsafeAttachment(t *testing.T) {
	...
	resp := performRequest(mux, http.MethodPost, "/v1/plugins/pharma_oa/api/suppliers", []byte(`{
		"code":"SUP001",
		"name":"Skoll Medical Supply",
