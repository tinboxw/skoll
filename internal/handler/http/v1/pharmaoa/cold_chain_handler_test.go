...
func TestColdChainHTTPRecordsJobsAndAnomalies(t *testing.T) {
	...
	contexts := contractHTTPRequest(mux, http.MethodGet, "/v1/pharma-oa/cold-chain-contexts", "", security.JWTClaims{Subject: "quality-user", Role: "quality"})
	...
	created := contractHTTPRequest(mux, http.MethodPost, "/v1/pharma-oa/cold-chain-records", createBody, security.JWTClaims{Subject: "quality-user", Role: "quality"})
	...
	run := contractHTTPRequest(mux, http.MethodPost, "/v1/pharma-oa/cold-chain-jobs", `{"minHumidityPercent":30,"maxHumidityPercent":70,"recipientId":"quality-manager","actorId":"spoofed"}`, security.JWTClaims{Subject: "quality-user", Role: "quality"})
	...
	anomalies := contractHTTPRequest(mux, http.MethodGet, "/v1/pharma-oa/cold-chain-anomalies?activeOnly=true", "", security.JWTClaims{Subject: "quality-user", Role: "quality"})
	...
	invalid := contractHTTPRequest(mux, http.MethodGet, "/v1/pharma-oa/cold-chain-records?limit=501", "", security.JWTClaims{Subject: "quality-user", Role: "quality"})
	if invalid.Code != http.StatusBadRequest {
		t.Fatalf("invalid limit status=%d body=%s", invalid.Code, invalid.Body.String())
	}
	retry := contractHTTPRequest(mux, http.MethodPost, "/v1/pharma-oa/cold-chain-jobs/cold-chain-job-1/retry", `{"actorId":"spoofed"}`, security.JWTClaims{Subject: "quality-user", Role: "quality"})
